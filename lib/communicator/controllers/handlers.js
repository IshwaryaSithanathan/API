...
class ReceiveMessageListener extends EventEmitter {}
const receiveMessageListener = new ReceiveMessageListener()
...
function send (message) {
  return new Promise(function (resolve, reject) {
    ...
    sendMessage(message)
    receiveMessageListener.once(messageType, function (replyMessage) {
      // callback
      messageType = null
  ...
  try {
    return new Promise(function (resolve, reject) {
      ...
      if (message.getType() === messageType) {
        receiveMessageListener.emit(message.getType(), message)
        return
      }
...
module.exports = function Handlers () {
  ...
  Handlers.prototype.send = function (message) {
    return send(message)
  }
}
